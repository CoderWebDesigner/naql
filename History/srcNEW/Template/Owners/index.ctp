<div class="owners index">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<tr class="sortHeader">
				<th></th>
				<th><?php echo $this->Paginator->sort('user_id', ___('إسم المستخدم')); ?></th>
			...
			<?php foreach ($owners as $i => $owner): ?>
				<tr>
					<td>
						<?php
						echo $this->AlaxosForm->checkBox('Owner.' . $i . '.id', array('value' => $owner->id, 'class' => 'model_id'));
						?>
					</td>
					<td>
						<?php echo $owner->has('user') ? $this->Html->link($owner->user->username, ['controller' => 'Users', 'action' => 'view', $owner->user->username]) : ''; ?>
					</td>
					<td>
						<?php echo h($owner->to_display_timezone('created')); ?>
					</td>
					...
					<td class="actions">
						...
						<?php 
						echo $this->Html->link('<span class="glyphicon glyphicon-search"></span>', ['action' => 'view', $owner->id], ['class' => 'to_view', 'escape' => false]);
						echo '&nbsp;&nbsp;';
						echo $this->Html->link('<span class="glyphicon glyphicon-pencil"></span>', ['action' => 'edit', $owner->id], ['escape' => false]);
						echo '&nbsp;&nbsp;';
						echo $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span>', ['action' => 'delete', $owner->id], ['confirm' => __('Are you sure you want to delete # {0}?', $owner->id), 'escape' => false]);
						?>
					</td>
				</tr>
			<?php endforeach; ?>
