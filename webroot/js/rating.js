    ...
    $.get("http://naql.codesroots.com/api/owners/ownerInfo/"+OwnerId+".json",
        function(data){
            ...
            if (commentedBefore == 1) {
                $('.postLoading').hide();
                $('#ratingResponse').html('<h5 class="order_text text-center p-2 p-lg-5">لقد قمت بالتقييم من قبل</h5>');
                $('#ratingResponse').fadeIn(2000).fadeOut(2000);
                setTimeout(function(){window.location = URL+"owners/myorders"}, 4000);
            }else {
                if(localStorage.getItem("userRate") == 0) {
                    $('.postLoading').hide();
                    $('#ratingResponse').html('<h5 class="order_text text-center p-2 p-lg-5">من فضلك ادخل تقييماً صحيحاً</h5>');
                    $('#ratingResponse').fadeIn(2000).fadeOut(2000);
                }
                else {
                    if($('#userComment').val() == ""){
                        $('.postLoading').hide();
                        $('#ratingResponse').html('<h5 class="order_text text-center p-2 p-lg-5">من فضلك ادخل تعليقاً صحيحاً</h5>');
                        $('#ratingResponse').fadeIn(2000).fadeOut(2000);
                    }
                    else {
                        ...
                        function(data , status){
                            $('.postLoading').hide();
                            $('#ratingResponse').html('<h5 class="text-center py-5">تم إضافة التقييم</h5>');
                            $('#ratingResponse').fadeIn(2000).fadeOut(2000);
                            setTimeout(function(){window.location = URL+"owners/myorders"}, 4000);
                        });
                    }
