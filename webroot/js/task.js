...
if (UserGroup == 1 || UserGroup == 3) {       // display current orders for "admin"
    ...
    function(data){
        ...
        $('<li class="" id="prev_page"><a class="page-link">السابق</a></li>').prependTo('#orders_pagination');
        $('#orders_pagination').append('<li class="" id="next_page"><a class="page-link">التالي</a></li>');
    ...
    function(data){
      ...
      {ownerID: OwnerId, status: 'completed'},
          function(data){
              ...
              // adding the next and the previous buttons
              $('<li class="previous" id="goto_prev_page"><a class="page-link">السابق</a></li>').prependTo('#previous_orders_pagination');
              $('#previous_orders_pagination').append('<li class="previous" id="goto_next_page"><a class="page-link">التالي</a></li>');
...
function createCurrentOrdersForAdmin(currentObj) {
    for(var i = 0 ; i < currentObj.length ; i++) {
        $('#currentOrders').append('<div class="container my-3" reservationID="'+ currentObj[i].id+'" id="admincontainer'+i+1+'"></div>');
        $('#admincontainer'+i+1).html('<div class="row bg-white orderContainer" id="adminrow'+i+1+'"></div>');
        $('#adminrow'+i+1).html('<div class="col-2 my-auto text-center p-1" id="admincol'+i+1+'"></div>'
            +'<div class="col-6 col-sm-7 text-right my-auto p-0" id="admincol'+i+2+'"></div>'
            +'<div class="col-4 col-sm-3 my-auto" id="admincol'+i+3+'"></div>'
            ...
            +'<div class="col-12 text-right"  dir="rtl" id="admincol'+i+9+'"></div>'
            +'<div class="col-12 text-center" dir="rtl" id="admincol'+i+10+'"></div>');
        $('#admincol'+i+1).html('<img src="'+URL+'images/earth_globe.png" '
            ...
            +'mapContainerID="div'+i+'" alt="Location" class="mapIcon map d-inline"><br>'
        ...
        $('#admincol'+i+2).html('<h5 class="order_main_username"><b>'+currentObj[i].user.username+'</b></h5>'
            +'<h6 class="order_main_orderID">طلب رقم : <span>'+currentObj[i].id+'</span></h6>');
        $('#admincol'+i+3).html('<div class="ownerImgContainer text-center">'
            +'<img src="http://naql.codesroots.com/library/profile/'+currentObj[i].user.photo+'" '
            +'alt="ProfileImage" class="ownerImage"></div>');
        $('#admincol'+i+4).html('<h6 class="order_text">تاريخ / وقت : <span dir="ltr">'+currentObj[i].date+'</span></h6>');
        $('#admincol'+i+5).html('<h6 class="order_text">نوع التعاقد : '+currentObj[i].reservation_type.name+'</h6>');
        $('#admincol'+i+6).html('<h6 class="order_text">طلب تم منذ : '
            ...
            +' ساعة </h6>');
        $('#admincol'+i+7).html('<div class="container m-0 p-0" id="admininternalContainer'+i+1+'"></div>');
        $('#admininternalContainer'+i+1).html('<div class="row my-auto m-0 p-0" dir="rtl" id="admininternalRow'+i+1+'"></div>');
        $('#admininternalRow'+i+1).html('<div class="col-1 text-left m-0 p-0">'
            +'<img src="'+URL+'images/solid-green-circle.png" alt="green bullet" class="circleBullet"></div>'
            +'<div class="col-11 my-1 p-0">'
            +'<h6 class="order_text px-2">نقطة الانطلاق : <span dir="rtl">'+currentObj[i].start_point+'</span></h6></div>'
            +'<div class="col-1 text-left m-0 p-0">'
            +'<img src="'+URL+'images/red-solid-circle.png" alt="red bullet" class="circleBullet"></div>'
            +'<div class="col-11 my-1 p-0">'
            +'<h6 class="order_text px-2">نقطة الوصول : <span dir="rtl">'+currentObj[i].end_point+'</span></h6></div>');
        $('#admincol'+i+9).html('<h6 class="order_text bg-light p-2 mx-1">تفاصيل الطلب : '+currentObj[i].description+'</h6>');

        // update the price zone. offer a price or wait customer approval
        var numberOfPrices = currentObj[i].owner_prices.length;
        if(numberOfPrices == 0){
            $('#admincol'+i+10).html('<hr>'
                +'<h6 class="order_text text-right m-0 p-1">من فضلك أدخل السعر المناسب : </h6>'
                ...
                +'<h6 class="order_text my-auto">ارسل عرض الطلب</h6></button>');}
        else {
            for(var ii = 0 ; ii<numberOfPrices ; ii++){
                if(currentObj[i].owner_prices[ii].owner_id == OwnerId){
                    $('#admincol'+i+10).html('<hr><h6 class="order_text text-center py-2 py-md-4">بانتظار موافقة العميل</h6>');
                    break;}
                else {
                  $('#admincol'+i+10).html('<hr>'
                      +'<h6 class="order_text text-right m-0 p-1">من فضلك أدخل السعر المناسب : </h6>'
    ...
    $('.sendRequest').click(function(){
        ...
        console.log("price input = " + $('#'+inputField).val());
        if(!$('#'+inputField).val()){
            $('#priceResponse').html('<h5 class="order_text text-center my-auto py-3 py-lg-5">من فضلك أدخل السعر الصحيح</h5>');
            $('#priceResponse').fadeIn(2000).fadeOut(2000);}
        else if($('#'+inputField).val() < 0) {
            $('#priceResponse').html('<h5 class="order_text text-center my-auto py-3 py-lg-5">من فضلك أدخل رقم أكبر من الصفر</h5>');
            $('#priceResponse').fadeIn(2000).fadeOut(2000);}
        else {
            ...
            function(status){
                $('#'+priceContainer).html('<hr><h6 class="order_text text-center py-2 py-md-4">بانتظار موافقة العميل</h6>');});
            }
        });
    ...
    $('.map').click(function(){
        ...
        else {$('#'+mapContainerID).hide();$(this).removeClass("clicked");}
    });
}
...
function createCurrentOrdersForOwner(currentObj) {
    for(var i = 0 ; i < currentObj.length ; i++){
        ...
        $('#owner_row'+i+1).html('<div class="col-2 my-auto text-center p-1" id="owner_col'+i+1+'"></div>'
            +'<div class="col-6 col-sm-7 text-right my-auto p-0" id="owner_col'+i+2+'"></div>'
            +'<div class="col-4 col-sm-3 my-auto" id="owner_col'+i+3+'"></div>'
            +'<div class="col-12 p-0 m-0"><div id="div'+i+'" class="mapContainer"></div></div>'
            ...
            +'<div class="col-12 text-right"  dir="rtl" id="owner_col'+i+9+'"></div>'
            +'<div class="col-12 text-center" dir="rtl" id="owner_col'+i+10+'"></div>');
        $('#owner_col'+i+1).html('<img src="'+URL+'images/earth_globe.png" id="map'+i+'" '
            ...
            +'mapContainerID="div'+i+'" alt="Location" class="mapIcon map d-inline">');
        $('#owner_col'+i+2).html('<h5 class="order_main_username"><b>'+currentObj[i].user.username+'</b></h5>'
            +'<h6 class="order_main_orderID">طلب رقم : <span>'+currentObj[i].id+'</span></h6>'
            +'<h6 class="order_text">نوع المعدة : <span>'+currentObj[i].machine_detail.name+'</span></h6>');
        $('#owner_col'+i+3).html('<div class="ownerImgContainer">'
            +'<img src="http://naql.codesroots.com/library/profile/'+currentObj[i].user.photo+'" '
            +'alt="ProfileImage" class="ownerImage"></div>');
        $('#owner_col'+i+4).html('<h6 class="order_text">تاريخ / وقت : <span dir="ltr">'+currentObj[i].date+'</span></h6>');
        $('#owner_col'+i+5).html('<h6 class="order_text">نوع التعاقد : '+currentObj[i].reservation_type.name_en+'</h6>');
        $('#owner_col'+i+6).html('<h6 class="order_text">طلب تم منذ : '
            +Math.floor((currentDate - Date.parse(currentObj[i].created))/(1000*60*60*24))
            +' يوم و '
            +Math.floor(((currentDate - Date.parse(currentObj[i].created))%(1000*60*60*24))/(1000*60*60))
            +' ساعة </h6>');
        $('#owner_col'+i+7).html('<div class="container m-0 p-0" id="owner_internalContainer'+i+1+'"></div>');
        $('#owner_internalContainer'+i+1).html('<div class="row my-auto m-0 p-0" dir="rtl" id="owner_internalRow'+i+1+'"></div>');
        $('#owner_internalRow'+i+1).html('<div class="col-1 text-left m-0 p-0">'
            +'<img src="'+URL+'images/solid-green-circle.png" alt="green bullet" class="circleBullet"></div>'
            +'<div class="col-11 m-0 p-0">'
            +'<h6 class="order_text px-2">نقطة الانطلاق : <span dir="rtl">'+currentObj[i].start_point+'</span></h6></div>'
            +'<div class="col-1 text-left m-0 p-0"><img src="'+URL+'images/red-solid-circle.png" alt="red bullet" class="circleBullet"></div>'
            +'<div class="col-11 my-1 p-0"><h6 class="order_text px-2">نقطة الوصول : <span dir="rtl">'+currentObj[i].end_point+'</span></h6></div>');
        $('#owner_col'+i+9).html('<h6 class="order_text bg-light p-2 mx-1">تفاصيل الطلب : '+currentObj[i].description+'</h6>');
        ...
        if(numberOfPrices == 0){
            ...
            for(var ii = 0 ; ii<numberOfPrices ; ii++){
                if(currentObj[i].owner_prices[ii].owner_id == OwnerId){
                    $('#owner_col'+i+10).html('<hr><h6 class="order_text text-center py-2 py-md-4">بانتظار موافقة العميل</h6>');
                    break;}
                else {
                  $('#owner_col'+i+10).html('<hr>'
                      +'<h6 class="order_text text-right m-0 p-1">من فضلك أدخل السعر المناسب : </h6>'
...
function createCurrentOrdersForUser(currentObj) {
    for(var i = 0 ; i < currentObj.length ; i++) {
        ...
        $('#user_container'+i+1).html('<div class="row bg-white orderContainer user_container" '
            +'id="user_row'+i+1+'" reservationID="'+ currentObj[i].id+'" ></div>');
        $('#user_row'+i+1).html('<div class="col-2 my-auto text-center p-1" id="user_col'+i+1+'"></div>'
            +'<div class="col-6 col-sm-7 text-right my-auto p-0" id="user_col'+i+2+'"></div>'
            +'<div class="col-4 col-sm-3 my-auto" id="user_col'+i+3+'"></div>'
            +'<div class="col-12 p-0 m-0"><div id="div'+i+'" class="mapContainer"></div></div>'
        ...
        $('#user_col'+i+1).html('<img src="'+URL+'images/earth_globe.png" id="map'+i+'" '
            ...
            +'alt="Location" class="mapIcon map d-inline">');
        $('#user_col'+i+2).html('<h5 class="order_main_username"><b>'+currentObj[i].machine_detail.name+'</b></h5>'
            +'<h6 class="order_main_orderID">طلب رقم : <span>'+currentObj[i].id+'</span></h6>');
        $('#user_col'+i+3).html('<div class="ownerImgContainer text-center">'
            ...
            +'alt="ProfileImage" class="ownerImage"></div>');
        $('#user_col'+i+4).html('<h6 class="order_text">تاريخ / وقت : <span dir="ltr">'+currentObj[i].date+'</span></h6>');
        $('#user_col'+i+5).html('<h6 class="order_text">نوع التعاقد : '+currentObj[i].reservation_type.name_en+'</h6>');
        $('#user_col'+i+6).html('<h6 class="order_text">طلب تم منذ : '
            +Math.floor((currentDate - Date.parse(currentObj[i].created))/(1000*60*60*24))
            +' يوم و '
            +Math.floor(((currentDate - Date.parse(currentObj[i].created))%(1000*60*60*24))/(1000*60*60))
            +' ساعة </h6>');
        $('#user_col'+i+7).html('<div class="container m-0 p-0" id="user_internalContainer'+i+1+'"></div>');
        $('#user_internalContainer'+i+1).html('<div class="row my-auto m-0 p-0" dir="rtl" id="user_internalRow'+i+1+'"></div>');
        $('#user_internalRow'+i+1).html('<div class="col-1 text-left m-0 p-0">'
            +'<img src="'+URL+'images/solid-green-circle.png" alt="green bullet" class="circleBullet"></div>'
            +'<div class="col-11 my-1 p-0">'
            +'<h6 class="order_text px-2">نقطة الانطلاق : <span dir="rtl">'+currentObj[i].start_point+'</span></h6></div>'
            +'<div class="col-1 text-left m-0 p-0">'
            +'<img src="'+URL+'images/red-solid-circle.png" alt="red bullet" class="circleBullet"></div>'
            +'<div class="col-11 my-1 p-0"><h6 class="order_text px-2">نقطة الوصول : <span dir="rtl">'+currentObj[i].end_point+'</span></h6></div>');
        $('#user_col'+i+9).html('<h6 class="order_text bg-light p-2 mx-1">تفاصيل الطلب : '+currentObj[i].description+'</h6>');}
...
function createPrevOrdersAdmin (obj){
    for(var i = 0 ; i<obj.length ; i++){
        ...
        $('#row'+i+1).html('<div class="col-2 my-auto text-center p-1" id="col'+i+2+'"></div>'
            +'<div class="col-6 col-sm-7 text-right my-auto p-0" dir="rtl" id="col'+i+3+'"></div>'
            +'<div class="col-4 col-sm-3 my-auto" id="col'+i+4+'"></div>'
            +'<div class="col-12 p-0 m-0"><div id="prev_div'+i+'" class="mapContainer"></div></div>'
            ...
            +'<div class="col-12 text-right"  dir="rtl" id="col'+i+8+'"></div>'
        ...
        $('#col'+i+2).html('<img src="'+URL+'images/earth_globe.png" '
                ...
                +'mapContainerID="prev_div'+i+'" alt="Location" class="mapIcon map d-inline">');
        $('#col'+i+3).html('<h5 class="order_main_username"><b>'+obj[i].user.username+'</b></h5>');
        $('#col'+i+4).html('<div class="ownerImgContainer text-center">'
            +'<img src="http://naql.codesroots.com/library/profile/'+obj[i].user.photo+'" '
            +'alt="ProfileImage" class="ownerImage"></div>');
        $('#col'+i+5).html('<div class="container p-0 m-0" id="internalContainerPrev'+i+1+'"></div>');
        $('#internalContainerPrev'+i+1).html('<div class="row my-auto m-0 p-0" dir="rtl" id="internalRowPrev'+i+1+'"></div>');
        $('#internalRowPrev'+i+1).html('<div class="col-1 text-left m-0 p-0">'
            +'<img src="'+URL+'images/solid-green-circle.png" alt="green bullet" class="circleBullet"></div>'
            +'<div class="col-11 text-right my-1 p-0">'
            +'<h6 class="order_text px-2">نقطة الانطلاق : <span dir="rtl">'+obj[i].start_point+'</span></h6></div>'
            +'<div class="col-1 text-left m-0 p-0"><img src="'+URL+'images/red-solid-circle.png" alt="red bullet" class="circleBullet"></div>'
            +'<div class="col-11 text-right my-1 p-0"><h6 class="order_text px-2">نقطة الوصول : <span dir="rtl">'+obj[i].end_point+'</span></h6></div>');
        $('#col'+i+6).html('<h6 class="order_text">نوع التعاقد : '+obj[i].reservation_type.name_en+'</h6>');
        $('#col'+i+7).html('<h6 class="order_text">تاريخ / وقت : <span dir="ltr">'+obj[i].date+'</span></h6>');
        $('#col'+i+8).html('<h6 class="order_text text-right bg-light p-2 mx-1">تفاصيل الطلب : '+obj[i].description+'</h6>');
        ...
        $('#internalRow'+i+1).html('<div class="col-10 text-right my-auto" id="internalCol'+i+2+'"></div>'
            +'<div class="col-2 text-left m-0 p-0" id="internalCol'+i+3+'"></div>');
        $('#internalCol'+i+2).html('<h6 class="order_text py-1">سعر التوصيل : <span>'+obj[i].price+'</span></h6>');
        $('#internalCol'+i+3).html('<img src="'+URL+'img/dollar.png" alt="price" class="dollarSign my-auto">');
    }
...
function createPrevOrdersOwner(obj) {
    for(var i = 0 ; i < obj.length ; i++){
        ...
        $('#row'+i+1).html('<div class="col-2 my-auto text-center p-1" id="col'+i+2+'"></div>'
            ...
            +'<div class="col-12 col-lg-6 text-right"  dir="rtl" id="col'+i+6+'"></div>'
            +'<div class="col-12 col-lg-6 text-right"  dir="rtl" id="col'+i+7+'"></div>'
            +'<div class="col-12 text-center"  dir="rtl" id="col'+i+8+'"></div>'
            +'<div class="col-12" id="col'+i+9+'"></div>');
        $('#col'+i+2).html('<img src="'+URL+'images/earth_globe.png" '
        +'id="prev_map'+i+'" start_lat="'+obj[i].start_lat+'" start_long="'+obj[i].start_long+'" '
        +'mapContainerID="prev_div'+i+'" alt="Location" class="mapIcon map d-inline">');
    ...
    // show or hide the start location on the map
    $('.map').click(function(){
        console.log("map is clicked in previous orders ")
        var mapContainerID = $(this).attr("mapContainerID");
        if($(this).hasClass("click")== false){
...
function createPrevOrdersUser (obj){
    for(var i = 0 ; i < obj.length ; i++){
        ...
        $('#container'+i+1).html('<div class="row bg-white orderContainer" id="row'+i+1+'"></div>');
        $('#row'+i+1).html('<div class="col-2 my-auto text-center p-1" id="col'+i+2+'"></div>'
            +'<div class="col-6 col-sm-7 text-right my-auto p-0" id="col'+i+3+'"></div>'
            +'<div class="col-4 col-sm-3 my-auto" id="col'+i+4+'"></div>'
            +'<div class="col-12 p-0 m-0"><div id="prev_div'+i+'" class="mapContainer"></div></div>'
            +'<div class="col-12 my-2" id="col'+i+5+'"></div>'
            +'<div class="col-12 col-lg-6 text-right"  dir="rtl" id="col'+i+6+'"></div>'
            ...
            +'<div class="col-12 text-right"  dir="rtl" id="col'+i+8+'"></div>'
            +'<div class="col-12" id="col'+i+9+'"></div>');
        $('#col'+i+2).html('<img src="'+URL+'images/inf.png" alt="" ownerID="'+obj[i].owner.id+'" machine_detail_id="'+obj[i].machine_detail_id+'" class="infoIcon driver_info pr-1">'
        ...
        $('#col'+i+3).html('<h5 class="order_main_username m-0 p-0"><b>'+ obj[i].owner.user.username +'</b></h5>'
            ...
            +'<p class="order_text p-0 m-0">(من <span>'+obj[i].owner.rates[0].count+'</span> عميل)</p>'
            +'</div></div></div>'
            +'<h6 class="order_text">رقم الطلب : '+ obj[i].id +'</h6>');
        $('#col'+i+4).html('<div class="ownerImgContainer text-center">'
        ...
        $('#col'+i+9).append('<div class="container m-0 p-0" id="internalContainer'+i+1+'"></div>');
        $('#internalContainer'+i+1).html('<div class="row m-0 p-0" id="internalRow'+i+1+'"></div>');
        $('#internalRow'+i+1).html('<div class="col-2 text-left my-auto py-1" id="internalCol'+i+1+'"></div>'
            +'<div class="col-8 text-right my-auto" id="internalCol'+i+2+'"></div>'
            +'<div class="col-2 text-left my-auto" id="internalCol'+i+3+'"></div>');
        $('#internalCol'+i+1).html('<img src="'+URL+'images/okSign.png" class="ratingIcon okSign" id="okSign-'+i+1+'" orderID="'+obj[i].id+'" ownerName="'+obj[i].owner.user.username+'" '
            +'orderDate=" '+obj[i].date+'" ownerID="'+obj[i].owner_id+'" ownerPhoto="'+obj[i].owner.user.photo+'" '
            +'startPoint="'+obj[i].start_point+'" endPoint="'+obj[i].end_point+'" userID="'+obj[i].user_id+'" alt="like">');
        $('#internalCol'+i+2).html('<h6 class="order_text py-1 my-auto">سعر التوصيل : <span>'+obj[i].price+'</span></h6>');
        $('#internalCol'+i+3).html('<img src="'+URL+'images/dollar.png" alt="price" class="dollarSign my-auto">');
